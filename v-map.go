...
// Map is like a Go map[any]*T but is safe for concurrent use
...
func (m *Map[T]) Load(key any) (value any, ok bool) {
	read := m.loadReadOnly()
	e, ok := read.m[key]
	if !ok && read.amended {
		m.mu.Lock()
...
func (m *Map[T]) LoadOrStore(key any, value *T) (actual any, loaded bool) {
...
func (m *Map[T]) CompareAndSwap(key, old, new *T) (swapped bool) {
	read := m.loadReadOnly()
	if e, ok := read.m[key]; ok {
...
func (m *Map[T]) CompareAndDelete(key, old *T) (deleted bool) {
	read := m.loadReadOnly()
	e, ok := read.m[key]
	if !ok && read.amended {
		m.mu.Lock()
